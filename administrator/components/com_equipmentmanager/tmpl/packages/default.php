...
{
	$saveOrderingUrl = 'index.php?option=com_equipmentmanager&task=packages.saveOrderAjax&tmpl=component&' . Session::getFormToken() . '=1';
	HTMLHelper::_('draggablelist.draggable');
}

?>
<form action="<?php echo Route::_('index.php?option=com_equipmentmanager'); ?>&view=packages" method="post" name="adminForm" id="adminForm">
	<div class="row">
		<div class="col-md-12">
...
//							$canEditOwn       = $user->authorise('core.edit.own',   'com_equipmentmanager.item.' . $item->id) && $item->created_by == $userId;
						...
						<tr class="row<?php echo $i % 2; ?>" data-draggable-group="<?php echo $item->catid; ?>">
							...
							<td class="text-center object-status">
								<?php echo HTMLHelper::_('jgrid.published', $item->published, $i, 'Packages.', $canChange, 'cb', $item->publish_up, $item->publish_down); ?>
							</td>
							<th scope="row" class="has-context">
								<?php if ($item->checked_out) : ?>
									<?php echo HTMLHelper::_('jgrid.checkedout', $i, $item->editor, $item->checked_out_time, 'items.', true); ?>
								<?php endif; ?>
								<a class="hasTooltip" href="<?php echo Route::_('index.php?option=com_equipmentmanager&task=package.edit&id=' . (int) $item->id); ?>" title="<?php echo Text::_('JACTION_EDIT'); ?> <?php echo $this->escape(addslashes($item->title)); ?>">
				...
				if ($user->authorise('core.create', 'com_equipmentmanager')
					...
					echo HTMLHelper::_(
						...
						'collapseModal',
						array(
							'title' => Text::_('COM_EQUIPMENTMANAGER_PACKAGES_BATCH_OPTIONS'),
